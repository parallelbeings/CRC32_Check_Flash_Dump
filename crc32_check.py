...
def calculate_crc_block():
    for i in range(0,0x1fffff,0x10000):  # Block 0 starting at address 0 to 0xffff
        ...
        out = s.find(check)
        #if out == 0:
        print("CRC for address range:", hex(offset), " - ", hex(offset + block_size), ":","CRC checksum:", ":", hex_out, "Check:",out)
...
def calculate_crc_sector():
    for i in range(0, 0x1fffff, 0x1000):  # Block 0 starting at address 0 to 0xffff
        ...
        #if out == -1:
        print("CRC for address range:", hex(offset), " - ", hex(offset + sector_size), ":","CRC checksum:", ":", hex_out, "Check:",out)
...
def calculate_crc_page():
    for i in range(0, 0x1fffff, 0x100):  # Block 0 starting at address 0 to 0xffff
        ...
        #if out == 0:
        print("CRC for address range:", hex(offset), " - ", hex(offset + page_size), ":","CRC checksum:", ":", hex_out, "Check:",out)
